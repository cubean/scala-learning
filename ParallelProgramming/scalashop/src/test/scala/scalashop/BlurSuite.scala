...
class BlurSuite extends FunSuite {
  ...
  test("boxBlurKernel should return the correct value on an interior pixel " +
    ...
    assert(boxBlurKernel(src, 1, 2, 3) === 10,
      s"(boxBlurKernel(1, 2, 3) should be 10, " +
        s"but it's ${boxBlurKernel(src, 1, 2, 3)})")
  }

  test("boxBlurKernel should return the correct value on an interior pixel " +
    "of a 3x4 image with 0, 0 radius 3") {
    val src = new Img(3, 4)
    src(0, 0) = 0; src(1, 0) = 1; src(2, 0) = 2
    src(0, 1) = 3; src(1, 1) = 4; src(2, 1) = 5
    src(0, 2) = 6; src(1, 2) = 7; src(2, 2) = 8
    ...
    assert(boxBlurKernel(src, 0, 0, 3) === 10,
      s"(boxBlurKernel(0, 0, 3) should be 10, " +
        s"but it's ${boxBlurKernel(src, 0, 0, 3)})")
  }
